...
from spikingjelly.activation_based import monitor, neuron, ann2snn
...
# test
# net.load_state_dict(torch.load("cfo_cnn.pth"))
print("Converting CNN to SNN")
model_converter = ann2snn.Converter(mode=1.0 / 3, dataloader=train_loader)
snn_net = model_converter(net)
snn_net.eval()
# torch.save(snn_net.state_dict(), "cfo_cnn2snn.pth")
test_outputs = []
test_labels = []
...
# fr_monitor = monitor.OutputMonitor(snn_net, neuron.IFNode, cal_firing_rate)
# fr_monitor.clear_recorded_data()

for inputs, labels in test_loader:
    inputs, labels = inputs.to(device), labels.to(device)

    test_outputs.append(scaler_y.inverse_transform(snn_net(inputs).cpu().detach().numpy()))
...
# layer_avg = np.mean(np.array(layer_avg).reshape(60, 4), 0)
